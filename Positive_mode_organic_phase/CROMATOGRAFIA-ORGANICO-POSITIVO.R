...
par(mfrow = c(1, 2))  # Define layout: 1 linha, 2 colunas
...
#O VALOR DE NOISE PARA DESCARTE DE RUIDO É DE 10000, pois apesar de aperecer um picos de ruido, eu consigo usar a função adjustedRtime. os brancos serão reduzidos com a matriz.
...
write_xlsx(isotope_free_matrix, "isotope_free_organic_positive.xlsx")
...
# Supondo que a primeira coluna de isotope_mplus e Pasta1 contenham os valores de m/z
mz_isotope <- isotope_mplus[, 1]  # Extraindo os m/z da matriz principal
mz_contaminantes <- Pasta1[, 1]   # Extraindo os m/z dos contaminantes

# Garantir que ambos os vetores são numéricos
mz_isotope <- as.numeric(mz_isotope)
mz_contaminantes <- as.numeric(mz_contaminantes)

indices_para_remover <- which(sapply(mz_isotope, function(x) 
  any(abs(x - mz_contaminantes) <= 0.005, na.rm = TRUE)))

isotope_mplus <- isotope_mplus[-indices_para_remover, ]
View(isotope_mplus)

# Supondo que as 6 primeiras amostras são os brancos

# Supondo que as colunas 10 a 15 são os brancos
nova_matrix <- isotope_mplus
...
nova_matrix <- nova_matrix[-c(1,2,13,14,20,21,22,27,28,29,35,36,37,46,47,51,53,54,55,56,57,60,62,63,64,65,68,70),]
...
# Aplicando o k-means nos resultados do t-SNE
set.seed(123)  # Para reprodutibilidade
kmeans_result3 <- kmeans(tsne_result3$Y, centers = 2)  
